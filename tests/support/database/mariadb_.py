...
class SupportMariadb(ISupportDb):
    @staticmethod
    ...
    @staticmethod
    def _get_tables(cursor: Cursor) -> set[str]:
        cursor.execute(
    ...
    @staticmethod
    def _get_system_tables(cursor: Cursor) -> set[str]:
        cursor.execute(
            "SELECT table_name FROM information_schema.tables WHERE table_schema = ? AND SUBSTR(TABLE_NAME FROM 1 FOR 13) = 'chronovoyage_'",
            (DEFAULT_TEST_ENV["MARIADB_DATABASE"],),
        )
    ...
    @classmethod
    def get_tables(cls):
        with cls.get_default_connection() as wrapper, wrapper.begin() as conn:
            cursor = conn.cursor()
            return cls._get_tables(cursor)
